...
/**
 * CDN keys of the 114 chapter artworks (the app module's SurahArtwork table
 * generates the same keys; kept in sync via scripts from the same asset
 * directory). The artwork ships from the asset CDN — see the surah_artwork
 * category in the shared manifest — and downloads on demand.
 */
internal val SURAH_ARTWORK_CDN_KEYS: List<String> = listOf(
...
internal fun surahArtworkCdnKey(surahNumber: Int): String? =
    SURAH_ARTWORK_CDN_KEYS.getOrNull(surahNumber - 1)

/**
 * Full-bleed chapter artwork header for the shared Quran screens. The iOS
 * actual resolves the CDN file (downloading on demand); other platforms that
 * don't host the shared screens render a neutral placeholder.
 */
@androidx.compose.runtime.Composable
internal expect fun SurahArtworkHeader(
    surahNumber: Int,
