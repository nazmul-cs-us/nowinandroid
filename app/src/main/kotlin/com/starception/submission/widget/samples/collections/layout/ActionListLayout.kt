...
import com.starception.submission.widget.samples.collections.layout.ActionListLayoutDimensions.verticalSpacing
import com.starception.submission.widget.samples.collections.layout.ActionListLayoutDimensions.widgetPadding
import com.starception.submission.widget.samples.collections.layout.ActionListLayoutSize.Companion.showTitleBar
import com.starception.submission.widget.samples.collections.layout.ActionListLayoutSize.Large
import com.starception.submission.widget.samples.collections.layout.ActionListLayoutSize.Small
 ...
 * @see [com.starception.submission.widget.samples.collections.CheckListAppWidget]
 *
 */
@Composable
...
private fun FilledActionListItem(
  ...
  fun StateIndicatorIcon() {
    Box(
      ...
      val bitmap = item.stateIconBitmap
      Image(
        provider = if (bitmap != null) ImageProvider(bitmap) else ImageProvider(item.stateIconRes),
        modifier = modifier.size(stateIconSize),
        contentDescription = null, // already covered in list item container's description
        // A supplied bitmap carries its own colour, so tinting it would flatten the
        // Meteocon to a single hue.
        colorFilter = if (bitmap != null) {
          null
        } else {
          ColorFilter.tint(
            if (isChecked) {
 ...
 * [com.starception.submission.widget.samples.collections.layout.ActionListLayout].
 *
...
data class ActionListItem(
  ...
  /**
   * Optional runtime-rendered icon that takes precedence over [stateIconRes].
   *
   * Added to the upstream layout so callers can supply artwork that has no drawable
   * resource — the prayer widget's Meteocons are rasterised from Lottie at load time.
   * A bitmap is also left untinted, since it arrives already coloured.
   */
  val stateIconBitmap: android.graphics.Bitmap? = null,
